-- =============================================================================
-- Dispatch Request Batch RPC Function
-- Atomic: locks request, transitions status, finds eligible providers, inserts dispatches
-- =============================================================================
-- Called from /api/request/send (batch 1) and /api/request/resend (batch 2+)

-- Ensure PostGIS types are accessible (Supabase hosts extensions in 'extensions' schema)
SET search_path TO public, extensions;

CREATE OR REPLACE FUNCTION public.dispatch_request_batch(
  p_request_id uuid,
  p_customer_id uuid,
...
BEGIN
  ...
  WITH eligible AS (
    ...
    WHERE r.id = p_request_id
      ...
      -- Never send the same request to the same provider twice
      AND NOT EXISTS (
        SELECT 1
